...
economist_data <- head(X1, country, HDI.Rank, HDI, CPI, Region)
```
...
```{r}
ggplot(data = economist_data) +
  geom_density(mapping = aes(x = Region))
  facet_wrap(~ Region, nrow=1)
```
**1.9 Show the distribution of `HDI` in each region using density plot. Set the transparency to 0.5**
```{r}
ggplot(data = economist_data) +
  geom_density(mapping = aes(fill = drv, x = HDI), alpha = 0.5)
```
**1.10 Show the distribution of `HDI` in each region using histogram and facetting.**
```{r}
ggplot(data = economist_data) +
 geom_histogram(mapping = aes(fill = drv, x = HDI), alpha = 0.5)
 facet_wrap(~ HDI, nrow=1)
```
...
```{r}
ggplot(data = economist_data) +
 geom_boxplot(mapping = aes(fill = drv, x = HDI, y = ), alpha = 0.5)
```
**1.12 Show the count of countries in each region using a bar plot.**
```{r}
ggplot(data = economist_data) +
 geom_bar(mapping = aes(x = countries))
```
